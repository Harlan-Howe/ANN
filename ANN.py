...
class ANN:
    ...
    def build_activation_functions_list(self, activation_ids: List[ActivationType]):
        ...
        if activation_ids is None:
            activation_ids = ActivationType.SIGMOID
        if isinstance(activation_ids, ActivationType):
            activation_ids = [activation_ids] * self.num_layers
            activation_ids = tuple(activation_ids)
        if (not isinstance(activation_ids, Tuple) or not isinstance(activation_ids[0], ActivationType) or
                len(activation_ids) != self.num_layers):
    ...
    def fromFile(cls, filename) -> "ANN":
        ...
        classmethod, which means that you call the class, ANN, instead of an object, and it makes an instance of the
        class.
    ...
    def predict(self, input_values: np.ndarray) -> np.ndarray:
        ...
        #  consider that the y-hat ("to axon") values for one layer plus a bias node become the x-values
        #  ("output_from_layer") for the next layer.
        #  There are three steps:
        #   1) Take the output (y_hat) from the previous layer and add a bias node to it to get the x_values for this
        #   layer. (Hint: there is a method in this class that does this.)
    ...
    def calculate_errors(self, expected_output: np.ndarray) -> List[np.ndarray]:
        """
        The first stage of backpropagation, we are measuring how much error there was as a measure of the input (a) to
        each layer. (Except for layer #0 - we assume the input was the real input!) This is going to be used to make
        corrections to the weight matrices. We start with the output layer, since that is the one we have information
        about what "should" have come out, and then we can use that information to figure out what "should" have
        happened at the previous layer, then the layer before that, etc. (Thus the "back" in "backpropagation.")
        :param expected_output: "y," the "actual" value that this ANN _should_ have output (y_hat) in the last
        predict() run.
        ...
        derivative_of_activation_function = (
            self.get_derivative_of_activation_for_layer_from_output(-1, self.y_hats_for_layers[-1]))

        # (Note: cellwise multiplication, not matrix mult.)
        ...
        #       1) apply (error_for_next_layer • weight_matrix^T) to get the difference at the "y-hat" level of this
        #       layer. This will be one longer than the number of nodes in this layer, because of the shape of the
        #       weight matrix.
        ...
        #   There is no error for the input layer, just the hidden ones and the output layer. (So these errors are
        #   always and only found for the input to the next layer after a weight matrix.)
    ...
    def update_weights(self, error_per_layer: List[np.ndarray], alpha: float = 0.02):
        """
        Uses the backpropagated errors sent from the weight matrices to each layer and the outputs (y_hat) from the
        previous layer that fed into that weight matrix to determine a correction to the weight matrix. This is
        multiplied by a factor, alpha, that determines the scale of the corrections and then used to change the weights.
        ...
        #  2) Use the a error matrix you found earlier (error_per_layer) for the "a" level of the next layer (size M)
        #  and that list of x values (size N+1) to generate a correction matrix, which is ((N+1) x M).
    ...
    @staticmethod
    def add_bias_node(y_hats: np.ndarray) -> np.ndarray:
        """
        Converts the y_hat array from one layer into the x array for the next layer by adding a 1 to the end of the
        array to represent the bias value.
    ...
    @staticmethod
    def strip_bias_node(array_to_strip: np.ndarray) -> np.ndarray:
        """
        ...
        :param array_to_strip: an N ndarray
    ...
    def get_derivative_of_activation_for_layer_from_output(self,
                                                           layer_num: int,
                                                           y_hat_values: np.ndarray) -> np.ndarray:
        """
    ...
    def visualize_prediction(self):
        ...
        for l in range(len(self.layer_sizes)):
            ...
            for n in range(self.layer_sizes[l]+1):
                ...
                cv2.circle(canvas, (int((l + 0.5) * pixels_per_layer), int((n + 0.5) * pixels_per_node[l])),
                           radius=int(NODE_DIAM / 2), color=(0.5, 0.5, 0.5), thickness=1)
...
if __name__ == "__main__":
    ...
    # output = a.apply_layer_forward(input_layer,0)
    # print (output)
    print("------")
