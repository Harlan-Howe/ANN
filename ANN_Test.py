...
class MyTestCase(unittest.TestCase):
    ...
    def test_2_a_and_y_hats(self):
        ...
        expected_as = [np.array([1., -0.5, 0., -0.5, -1.]),
                       np.array([1.67219403, 1.98473154, 1.39369625, 1.54398201, 1.94811212, 2.06485024, 2.02812228,
                                 1.60509281]),
                       np.array([4.32923312, 3.6118534, 2.78584314, 2.87044873, 2.55395664, 3.23304541]),
                       np.array([3.73886022, 2.91403626, 4.59257246])]
        expected_y_hats = [np.array([0.73105858, 0.37754067, 0.50000000, 0.37754067, 0.26894142]),
                           np.array([0.84186812, 0.87918464, 0.80118167, 0.82404285, 0.87524064, 0.88743957, 0.88371826,
                                     0.83272897]),
                           np.array([0.98699374, 0.97370817, 0.94190600, 0.94636613, 0.92783888, 0.96205907]),
        ...
        for i in range(len(expected_y_hats)):
            np.testing.assert_almost_equal(actual=self.myANN.y_hats_for_layers[i],
                                           desired=expected_y_hats[i],
                                           decimal=5)
    ...
    def test_5_backpropagated_error_levels(self):
        ...
        errs = self.myANN.calculate_errors(actual_y)
        expected5 = [np.array([9.98982418e-05, 9.80256732e-05, 2.85118269e-04, 1.30073861e-04, 1.96744536e-04,
                               2.01665489e-04, 1.27920832e-04, 2.22612083e-04]),
    ...
    def test_6_new_weights_after_learning_once(self):
        print("test 6")
        # We need to run the prediction, but we're not actually using the result, so output is just a dummy variable.
        ...
        expected6 = [np.array([[0.85073197, 0.96268444, 0.68371394, 0.62047001, 0.67048640, 0.22771786, 0.11057179,
                                0.31151796],
                               [0.47249964, 0.50003416, 0.34801007, 0.27623196, 0.97785393, 0.51606713, 0.31444752,
                                0.43208772],
                               [0.49555375, 0.44620003, 0.88029283, 0.54262062, 0.53151932, 0.93082745, 0.58362098,
                                0.93961588],
                               [0.98687875, 0.62226481, 0.54532030, 0.75350673, 0.96538022, 0.66656106, 0.77838039,
                                0.20575183],
                               [0.53367550, 0.45880161, 0.19041776, 0.58891099, 0.13019126, 0.04723732, 0.96361536,
                                0.24338234],
                               [0.10806023, 0.51079105, 0.06550235, 0.27200887, 0.42367566, 0.97396954, 0.98385222,
                                0.60149278]]),
                     np.array([[0.99670001, 0.26399186, 0.22647327, 0.07071475, 0.21237145, 0.21307201],
                               [0.11310201, 0.44489730, 0.08090161, 0.20797052, 0.01739302, 0.87906721],
                               ...
                               [0.88063541, 0.61637980, 0.81909518, 0.37181074, 0.19842818, 0.50530273],
                     ...
                     np.array([[0.33796321, 0.42729125, 0.77881917],
                               ...
                               [0.96571600, 0.07654822, 0.39422035],
    ...
    def test_7_new_prediction_after_learning_once(self):
        print("test 7")
        # We need to run the prediction, but we're not actually using the result, so output2 is just a dummy variable.
        ...
        np.testing.assert_almost_equal(actual=self.myANN.predict(np.array((-1.25, -8, 2.25, 1.5, 4.75))),
                                       desired=expected7, decimal=5)
