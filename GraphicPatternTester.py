...
class GraphicPatternTester:
    ...
    def generate_data(self, N: int, num_bins: int = 4, which_pattern: int = 0) \
    ...
    @staticmethod
    def generate_spiral_data(N: int, num_bins: int = 4) \
            -> Tuple[List[np.ndarray], List[np.ndarray]]:
        pts = []
        one_hot_out = []
        for count in range(N):
            ...
            bin_choice = int((angle + math.pi) / (2 * math.pi) * num_bins + r / 3) % num_bins
            input_value = np.array([x, y])
            output_value = np.zeros([num_bins])
            output_value[bin_choice] = 1
            pts.append(input_value)
            one_hot_out.append(output_value)

        return pts, one_hot_out

    @staticmethod
    def generate_wave_data(N: int, num_bins: int = 4) \
            -> Tuple[List[np.ndarray], List[np.ndarray]]:
        pts = []
        one_hot_out = []
        for count in range(N):
            ...
            bin_choice = val % num_bins
            input_value = np.array([r, c])
            output_value = np.zeros([num_bins])
            output_value[bin_choice] = 1
            pts.append(input_value)
            one_hot_out.append(output_value)
    ...
    @staticmethod
    def generate_dots_data(N: int, num_bins: int = 4) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        pts = []
        one_hot_out = []
        ...
        for count in range(N):
            ...
            y = random.random()*20-10
            angle = math.atan2(y, x)
            bin_choice = int((angle + math.pi) / (2 * math.pi) * num_bins) % num_bins
            nearest_dot_angle = ((bin_choice + 0.5) * math.pi * 2) / num_bins - math.pi
            ...
            if r_squared < threshold_squared:
                bin_choice = (bin_choice + 2) % num_bins
            input_value = np.array([x, y])
            output_value = np.zeros([num_bins])
            output_value[bin_choice] = 1
            pts.append(input_value)
    ...
    @staticmethod
    def draw_sample(canvas, loc, one_hot, N=-1, x_offset=0, y_offset=0):
        """
        ...
        :param canvas: the ndarray image into which we are drawing.
        ...
        for count in range(N):
            canvas[y_offset + int(5 * (loc[count][0] + 10))][x_offset + int(5 * (loc[count][1] + 10))] = \
                colors[np.argmax(one_hot[count])]
...
if __name__ == "__main__":
    ...
    myAnn = ANN([2, 20, 10, 10, NUM_BINS], activation_ids=[ActivationType.IDENTITY,
    ...
    cv2.putText(image, f"<-- test results. ({int(percent_correct_tested)}%) Press any key to continue.",
                (120, 550), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(1, 1, 1))
